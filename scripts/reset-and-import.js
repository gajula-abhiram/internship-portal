...
/**
 * Reset and initialize database with full schema
 */
function resetDatabase() {
  ...
  for (const table of tables) {
    try {
      ...
      console.warn(`  - Failed to drop ${table.name}: ${error.message}`);
    }
  }
  
  // Execute the schema to create all tables
  console.log('🏗️ Creating tables with full schema...');
  db.exec(schema);
...
async function importSyntheticData() {
  try {
    ...
    // Reset database with full schema first
    ...
    for (const student of students) {
      try {
        db.prepare(`
          INSERT INTO users (username, password_hash, role, name, email, department, current_semester, cgpa, phone, placement_status)
          VALUES (?, ?, 'STUDENT', ?, ?, ?, ?, ?, ?, 'AVAILABLE')
        `).run(
          `student_${student.id}`,
          defaultPasswordHash,
          student.name,
    ...
    for (const mentor of mentors) {
      try {
        db.prepare(`
          INSERT INTO users (username, password_hash, role, name, email, department, placement_status)
          VALUES (?, ?, 'MENTOR', ?, ?, ?, 'AVAILABLE')
        `).run(
          `mentor_${mentor.id}`,
          defaultPasswordHash,
          mentor.name,
    ...
    for (const employer of employers) {
      try {
        db.prepare(`
          INSERT INTO users (username, password_hash, role, name, email, placement_status)
          VALUES (?, ?, 'EMPLOYER', ?, ?, 'AVAILABLE')
        `).run(
