 ...
 * Approve application (mentors only)
 */
export const PUT = withAuth(async (req: AuthenticatedRequest, context: { params: { id: string } }) => {
  try {
    ...
    // Check if mentor can approve (same department)
    if (application.student_department !== user.department) {
      return ApiResponse.forbidden('You can only approve applications from your department');
    }
    ...
    const updateApplication = db.prepare(`
      UPDATE applications 
      SET status = 'MENTOR_APPROVED', mentor_id = ?, mentor_approved_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `);
    ...
    return ApiResponse.serverError('Failed to approve application');
  }
}, ['MENTOR']);